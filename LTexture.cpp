...
#endif
#if defined(SDL_TTF_MAJOR_VERSION)
bool LTexture::loadFromRenderedText1( std::string textureText, SDL_Color textColor,SDL_Renderer*& gRenderer, TTF_Font*&  gFont_HighScore  )
{
	//Get rid of preexisting texture
	free();

	//Render text surface
	SDL_Surface* textSurface = TTF_RenderText_Solid( gFont_HighScore, textureText.c_str(), textColor );
	if( textSurface != NULL )
	{
		//Create texture from surface pixels
        mTexture = SDL_CreateTextureFromSurface( gRenderer, textSurface );
